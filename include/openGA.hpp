...
// https://github.com/Arash-codedev/openGA/blob/master/LICENSE

// Modifications by Guillermo Legarda Herranz are delimited by
//
//    //========== GLH ==========//
//    //=========================//
//
...
struct GenerationType
{
	...
	//========== GLH ==========//
	float parsimony_coefficient = 0.0;
	//=========================//
...
public:
	...
	//========== GLH ==========//
	function<void(thisGenerationType&)> calculate_parsimony_coefficient;
	// (Add float argument to calculate_SO_total_fitness)
	function<double(const thisChromosomeType&, float)> calculate_SO_total_fitness;
	//=========================//
	...
	Genetic() :
		...
		//========== GLH ==========//
		calculate_parsimony_coefficient(nullptr),
		//=========================//
...
protected:
	...
	void finalize_generation(thisGenerationType &new_generation)
	{
		...
		{
			//========== GLH ==========//
			// In initialisation of 'double best' and 
			// 'double current_cost', 'total_cost' has
			// been replaced with 'middle_costs.fitness' 
			// and made negative (fitness is positive,
			// but openGA minimises costs)
			//=========================//

			double best=-new_generation.chromosomes[0].middle_costs.fitness;
			double sum=0;
			new_generation.best_chromosome_index=0;

			for(unsigned int i=0;i<new_generation.chromosomes.size();i++)
			{
				double current_cost=-new_generation.chromosomes[i].middle_costs.fitness;
				sum+=current_cost;
	...
	****************************************************/
	...
	void finalize_objectives(thisGenerationType &g)
	{
		...
		switch(problem_mode)
		{
			case GA_MODE::SOGA:
				//========== GLH ==========//
				calculate_parsimony_coefficient(g);
				// (Add parsimony coefficient argument)
				for(int i=0;i<int(g.chromosomes.size());i++)
					g.chromosomes[i].total_cost=calculate_SO_total_fitness(g.chromosomes[i], g.parsimony_coefficient);
				//=========================//
				break;
			case GA_MODE::IGA:
